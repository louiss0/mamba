...
void main() {
  ...
  group("ToBashCompletionConverter", () {
    test('converts flags', () {
      final converter = ToBashCompletionConverter(
        CommandRegistry.create(
          'curl',
          'Do http requests',
          flags: [
            BooleanFlag('show-headers', short: "i"),
            BooleanFlag('silent', short: "-s"),
            BooleanFlag('remote-name', short: "O"),
      ...
      expect(
        ...
        equals(r'''
          ...
          declare -a global_flags=('-h' '--help' '--show-headers' '-i' '--silent' '-s' '--remote-name' '-V' '--verbose' 'V')
          ...
          _curl_completion() {
            ...
            COMPREPLY=($(compgen -W "${global_flags[*]}" -- "$current"))
          }
    ...
    test('converts choice positionals', () {});
    test('converts repeated choice positionals', () {});
    test('converts variadics', () {});

    test('converts options', () {
      final converter = ToBashCompletionConverter(
        CommandRegistry.create(
          'curl',
          'Do http requests',
          options: [
            StringOption('user', short: "u"),
            StringOption('upload-file', short: "T"),
            StringOption('output', short: "o"),
            StringOption("user-agent", short: "A"),
          ],
        ).toMap(),
      );
      expect(
        ...
        equals(r'''
          ...
          declare -a global_flags=('-h' '--help' )

          declare -a global_options=('-u' '--user' '-T' 'upload-file' '-o' '--output' '-A' '--user-agent')
          ...
          _curl_completion() {
            ...
            COMPREPLY=($(compgen -W "${global_flags[*]}  ${global_options[*]}" -- "$current"))
          }
    ...
    test('converts int options with range', () {});

    test('converts double options with range', () {});

    test('converts accessors', () {});

    group('converts nested accessors', () {});
    ...
    test('converts commands', () {
      final converter = ToBashCompletionConverter(
        CommandRegistry.create(
          ...
          commands: [TestCommand("commit", "Send a file into the git index")],
        ).toMap(),
      );
      ...
      expect(
        ...
        equals(r"""
          ...
          _git_completion() {
            ...
            local -A commands=(
              ['commit']='_git_commit_completion'
            )

            local command="${commands[$prev]}"
    ...
    group('converts nested commands', () {
      test('converts commands', () {
        final randomComamndNames = ['one', 'two', 'three', 'four'];

        final converter = ToBashCompletionConverter(
          CommandRegistry.create(
            'tool',
            'Work seamlessly with GitHub from the command line',
            commands: randomComamndNames.indexed
                .map(
                  (entry) => TestGroupCommand(
                    entry.$2,
                    randomComamndNames
                        .sublist(entry.$1)
                        .map(
                          (name) => TestGroupCommand(
                            ...
                            randomComamndNames
                                ...
                                .map(
                                  (name) =>
                                      TestCommand(name, 'This is the $name'),
                                )
                                .toList(),
                            'This is the group command $name',
                          ),
                        )
                        .toList(),
                    'This is the command ${entry.$2}',
                  ),
                )
                .toList(),
          ).toMap(),
        ...
        final randomCommandNamesStructure = randomComamndNames.indexed.expand((
          entry,
        ) {
          final descendantNames = randomComamndNames.sublist(entry.$1);
          return [
            [entry.$2],
            ...descendantNames.expand(
              (groupName) => [
                [entry.$2, groupName],
                ...descendantNames.map(
                  (commandName) => [entry.$2, groupName, commandName],
                ),
              ],
            ),
          ];
        }).toList();
        final commandCompletions = randomCommandNamesStructure
            .map((commandNames) => commandNames.join('_'))
            .map(
              (commandName) =>
                  """
            _tool_${commandName}_completion() {
              COMPREPLY=()
        ...
        expect(
          ...
          equals("""
            ...
            _tool_completion() {
              ...
              local -A commands=(
                ${randomCommandNamesStructure.map((commandNames) => "['${commandNames.join(' ')}']='_tool_${commandNames.join('_')}_completion'").join('\n              ')}
              )
