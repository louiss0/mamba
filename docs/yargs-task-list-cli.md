# Yargs task-list CLI decisions

This document explains the command design for the JSON-backed task list in
`bin/yargs_task_list.dart`. It is a separate executable from the existing
`task_list` ArgParser example, and is built on this package's low-level
`YargsParser` port.
...
```text
yargs_task_list add <title> [--description <description>]
yargs_task_list delete <id> | --completed
yargs_task_list update <id> (--title <title> | --description <description> | --complete <true|false>)
yargs_task_list list [--status all|complete|incomplete]
```
...
available actions from the root command list.
...
The `executables` entry in `pubspec.yaml` also exposes it as `yargs_task_list` when
the package is installed globally.

## Storage location and format

The default file is `.yargs_tasks.json` in the current working directory. A
separate filename prevents this YargsParser example from reading or modifying
the original `task_list` CLI's `.tasks.json` data. Like the original, it is
project-local, easy to inspect, back up, and remove.
...
```text
yargs_task_list add "Write release notes"
yargs_task_list add "Write release notes" --description "Summarize changes"
```
...
```text
yargs_task_list delete 14
yargs_task_list delete --completed
```
...
```text
yargs_task_list update 14 --title "Publish release notes"
yargs_task_list update 14 --description "Include migration guidance"
yargs_task_list update 14 --complete=true
yargs_task_list update 14 --complete=false
```
...
```text
yargs_task_list list
yargs_task_list list --status=complete
yargs_task_list list --status=incomplete
```

`list` defaults to `all`. Status is a named filter rather than separate
commands because it changes how the same resource is viewed. The command
accepts only `all`, `complete`, or `incomplete`; it never silently accepts an
unknown state.
...
## Yargs parser integration

`YargsParser` is intentionally only a token decoder: unlike the older
schema-driven `ArgParser`, it does not register commands. `YargsTaskListCli` therefore
uses the first token as the command and parses the remaining tokens with that
command's `YargsParserOptions`. This keeps command dispatch visible in the CLI
while still using the new parser for every flag, positional, default, and type
conversion.

Each command registers only its own known keys. It enables
`unknownOptionsAsArgs`, so an unrecognized flag is returned with the positional
values and is rejected by the command's positional-shape validation rather
than becoming an unnoticed property. It also disables duplicate-value arrays
and positional number conversion: task text and IDs remain single strings, and
the CLI explicitly converts an ID only after selecting an ID-taking command.

The generic parser handles token syntax, Boolean conversion, string options,
and the list-status default. The command layer handles task-specific rules that
span more than one input:
...
application-specific behavior and makes the policy for this small CLI explicit.
